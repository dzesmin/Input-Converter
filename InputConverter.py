...
def MPI_call_DEMC(noSpecies, MadhuPT):
     '''
     ...
     T0   , temperature at the top of the atmosphere
...
def PT_generator(p, free_params, noSpecies, MadhuPT):
     ...
     # if sys.argument(3) defines inversion case, returns inverted PT profile
     if MadhuPT == 'MadhuPT_Inv':

          # pulls out 6 parameters for PT profile, inversion case
          a1, a2, p1, p2, p3, T0 = free_params[noSpecies:noSpecies+6]

          # call PTatmfile.py module, PT_Inversion function
          PT_Inv, T_smooth_Inv = PT_Inversion(p, a1, a2, p1, p2, p3, T0)
          return PT_Inv, T_smooth_Inv


     # if sys.argument(3) defines non-inversion case, returns non-inverted PT profile
     elif MadhuPT == 'MadhuPT_NoInv':

          # pulls out 5 parameters for PT profile, non-inversion case
          a1, a2, p1, p3, T0 = free_params[noSpecies:noSpecies+5]

          # call PTatmfile.py module, PT_NoInversion function
          PT_NoInv, T_smooth_NoInv = PT_NoInversion(p, a1, a2, p1, p3, T0)
...
# plots PT profiles
def plot_PT(free_params, p, PT, T_smooth, MadhuPT):
     '''
      This function plots two figures:
      1.
      Madhu's PT profile for inversion or non-inversion case
      based on the string in sys.argument(3). It uses returned arrays from
     ...
     free_params   : fmol1, fmol2, fmol3..., a1, a2, p1, p2, p3, T0
     ...
     if MadhuPT == 'MadhuPT_Inv':
          ...
          T, T1, T2, T3 = PT[8], PT[9], PT[10], PT[11]

          # takes T0 from free_params
          T0 = free_params[9]
...
def MPI_call_Transit(free_params, p, PT, T_smooth, Transit_input, MadhuPT):
     ...
     # plots PT profile
     plot_PT(free_params, p, PT, T_smooth, MadhuPT)
...
def loop(p, noSpecies, MadhuPT):
     ...
     while(True):
          ...
          free_params = MPI_call_DEMC(noSpecies, MadhuPT)
...
def main():
     '''
     This function defines number of arguments, reads the atmospheric file given,   
     extracts list of pressures from the atmospheric file, and calls the loop to
     generate Transit input. 
     ...
     arg(1), atmospheric file made in a format that Transit code uses (Patricio Rojo)
     arg(2), integer number of molecular species
     arg(3), to define whether it is a case of Madhu's inverted or non-invereted 
       atmosphere by specifying the string "MadhuPT_Inv" for inverted atmosphere
       and the string "MadhuPT-NoInv" for non-inverted atmosphere. 
     ...
     if noArguments != 4:
        print '\nUsage: InputConverter.py atmfile number_of_species MadhuPT_Inv/MadhuPT_NoInv'

        print '\nExample for an inverted atmosphere:\nInputConverter.py atmosphJB.dat 4 MadhuPT_Inv\n'
        return
     ...
     if sys.argv[3] != 'MadhuPT_Inv' and sys.argv[3] != 'MadhuPT_NoInv':
        print '\nUsage: InputConverter.py atmfile number_of_species MadhuPT_Inv/MadhuPT_NoInv'
